...
public class ExamPdfService {
    ...
    public byte[] generate(ExamEvaluationDto dto) {
        ...
        try (PDDocument document = new PDDocument();
            ...
            // UPDATED: PdfLayoutEngine now loads fonts internally
            ...
            for (QuestionEvaluationDto q : dto.evaluatedQuestions()) {
                ...
                if (q.rubricReference() != null
                        && q.rubricReference().band() != null) {

                    bandRange =
                            q.rubricReference().band().min()
                                    + " - "
                                    + q.rubricReference().band().max();
                }
            ...
            for (QuestionEvaluationDto q : dto.evaluatedQuestions()) {
                ...
                pdf.section("Evaluation Summary",
                        q.evaluationSummary());

                pdf.list("Coverage Gaps", q.coverageGaps());
                pdf.list("Factual Errors", q.factualErrors());

                pdf.section("Student Answer",
                        q.studentAnswerTranscription());
